...
package com.cioinfotech.cychat.features.login.adapter
...
class ServerListAdapter : RecyclerView.Adapter<ServerListAdapter.ServerListViewHolder>() {
    private var serverList = mutableListOf<Group>()
    ...
    override fun onBindViewHolder(holder: ServerListViewHolder, position: Int) {
        holder.itemBinding.tvServerName.text = serverList[position].desc
        holder.itemBinding.root.setOnClickListener {
            itemClickListener?.onClick(serverList[position])
        }
    }

    fun updateData(list: MutableList<Group>) {
        serverList = list
