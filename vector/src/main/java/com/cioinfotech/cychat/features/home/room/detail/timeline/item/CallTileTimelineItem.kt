...
import com.cioinfotech.cychat.core.extensions.setLeftDrawable
...
abstract class CallTileTimelineItem : AbsBaseMessageItem<CallTileTimelineItem.Holder>() {
    ...
    private fun renderMissedStatus(holder: Holder) {
        ...
        if (attributes.informationData.sentByMe) {
            ...
            if (attributes.callKind.isVoiceCall) {
                holder.statusView.setStatus(R.string.call_tile_voice_missed, R.drawable.quantum_ic_call_missed_white_24)
            } else {
                holder.statusView.setStatus(R.string.call_tile_video_missed, R.drawable.quantum_ic_call_missed_white_24)
            }
        }
        holder.acceptRejectViewGroup.isVisible = true
        holder.acceptView.setText(R.string.call_tile_call_back)
        holder.acceptView.setLeftDrawable(attributes.callKind.icon, R.color.white)
    ...
    private fun renderInCallStatus(holder: Holder) {
        ...
        when {
            attributes.callKind == CallKind.CONFERENCE -> {
                ...
                holder.rejectView.setLeftDrawable(R.drawable.ic_call_hangup, R.color.white)
    ...
    private fun renderInvitedStatus(holder: Holder) {
        when {
            attributes.callKind == CallKind.CONFERENCE                       -> {
                ...
                holder.acceptView.setLeftDrawable(R.drawable.ic_call_video_small, R.color.white)
            }
            !attributes.informationData.sentByMe && attributes.isStillActive -> {
                ...
                if (attributes.callKind == CallKind.AUDIO) {
                    ...
                    holder.acceptView.setLeftDrawable(R.drawable.ic_call_audio_small, R.color.white)
        ...
        when {
            ...
            attributes.callKind == CallKind.CONFERENCE -> holder.statusView.setStatus(R.string.call_tile_video_active)
            attributes.informationData.sentByMe        -> holder.statusView.setStatus(R.string.call_ringing)
            attributes.callKind.isVoiceCall            -> holder.statusView.setStatus(R.string.call_tile_voice_incoming)
            else                                       -> holder.statusView.setStatus(R.string.call_tile_video_incoming)
        }
    }
    ...
    enum class CallKind(@DrawableRes val icon: Int, @StringRes val title: Int) {
        ...
        val isVoiceCall get() = this == AUDIO
    }
